...
# Custom Styling for clean Mobile/Desktop view
st.markdown("""
    <style>
    .main-title {
        text-align: center;
...
def parse_pipe_delimited(lines):
    ...
    for l in lines:
        stripped = l.strip()
        if not stripped or re.match(r'^-{10,}', stripped) or stripped.startswith('^[') or stripped.startswith('\x0c'):
            continue
        ...
        if '|' in stripped:
            ...
            if parts and parts[-1] == "":
                parts = parts[:-1]
            if not header_cols and any(w in stripped.upper() for w in ["NAME", "ACCOUNT", "AMOUNT", "LIMIT", "DATE", "PRODUCT"]):
                header_cols = parts
...
def parse_cbs_fixed_report(lines):
    ...
    if header_idx == -1:
        for i, l in enumerate(lines):
            if i + 1 < len(lines) and re.match(r'^\s*-{15,}', lines[i+1]) and len(l.strip()) > 10:
                header_idx = i
                break
    if header_idx == -1:
        return None
    ...
    header_matches = list(re.finditer(r'(\S+(?:\s(?!\s)\S+)*)', raw_header_line))
    ...
    for l in lines[header_idx + 2:]:
        ...
        if report_title and report_title.upper() in stripped.upper():
            continue
        if stripped == raw_header_line.strip() or ("ACCOUNT NUMBER" in stripped and "CUSTOMER NAME" in stripped):
            continue
        ...
        if len(parts) == len(col_headers):
            row_dict = {col_headers[k]: clean_dr_amount(parts[k]) for k in range(len(col_headers))}
            data_rows.append(row_dict)
        elif len(parts) >= 2:
            row_dict = {}
            for k in range(len(col_headers)):
                st_pos = col_starts[k]
                en_pos = col_starts[k+1] if k+1 < len(col_starts) else None
                st_adj = max(0, st_pos - 2) if k > 0 else 0
                val = l[st_adj:en_pos].strip() if en_pos and len(l) >= st_adj else (l[st_adj:].strip() if len(l) >= st_adj else "")
...
def parse_csv_generic(content_str):
    ...
    try:
        df = pd.read_csv(io.StringIO(content_str), sep=delim, engine='python', on_bad_lines='skip')
    except Exception:
        return None
...
uploaded_files = st.file_uploader(
    "મોબાઇલ કે કમ્પ્યુટરમાંથી ટેક્સ્ટ ફાઇલો પસંદ કરો (Select .txt / .csv files):",
    accept_multiple_files=True,
    type=["txt", "csv", "dat", "prn"]
)
...
if uploaded_files:
    ...
    if st.button("🚀 Convert to Excel (એક્સેલમાં કન્વર્ટ કરો)"):
        ...
        if failed_files:
            st.warning(f"⚠️ સ્કીપ થયેલ ફાઇલો ({len(failed_files)}): {', '.join(failed_files)}")
        ...
        if len(converted_files) == 1:
            ...
            st.download_button(
                label=f"📦 Download All Excel Files (.ZIP - {len(converted_files)} Files)",
